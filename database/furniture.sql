...
CREATE TABLE products (
    ...
    product_status ENUM('AVAILABLE', 'RESERVED', 'SOLD') DEFAULT 'AVAILABLE',
...
CREATE TABLE orders (
    ...
    order_status ENUM('PENDING', 'DELIVERING', 'DELIVERED', 'CANCELLED') DEFAULT 'PENDING',
...
WHERE p.is_disabled = FALSE AND a.is_disabled = FALSE;
...
CREATE VIEW view_all_discounts AS 
SELECT * FROM discounts;

CREATE VIEW view_active_discounts AS
SELECT * FROM discounts WHERE is_disabled = FALSE;
...
BEGIN
    RETURN CASE p_entity
        WHEN 'category' THEN EXISTS (
            SELECT 1 FROM categories WHERE category_id = p_id AND is_disabled = FALSE
        )
        WHEN 'brand' THEN EXISTS (
            SELECT 1 FROM brands WHERE brand_id = p_id AND is_disabled = FALSE
        )
        WHEN 'collection' THEN EXISTS (
            SELECT 1 FROM collections WHERE collection_id = p_id AND is_disabled = FALSE
        )
        ELSE FALSE
    END;
END //
...
BEGIN 
    IF p_variant_ref IS NULL OR p_variant_ref = '' THEN 
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Variant_ref cannot be empty';
    END IF;

    IF NOT is_valid_entity(p_category_id, 'category') THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid or disabled category';
    END IF;

    IF NOT is_valid_entity(p_brand_id, 'brand') THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid or disabled brand';
    END IF;

    IF NOT is_valid_entity(p_collection_id, 'collection') THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid or disabled collection';
    END IF;

    INSERT INTO products (
        product_name, product_description, category_id, brand_id, collection_id, variant_ref
    ) VALUES (
        p_name, COALESCE(p_description, 'No description available'), p_category_id, p_brand_id, p_collection_id, p_variant_ref
    );
END //
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM products WHERE product_id = p_product_id AND is_disabled = FALSE
    ) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Product not found or disabled';
    END IF;
...
BEGIN
    ...
    SET sql_stmt = CONCAT('INSERT INTO ', p_table_name, ' (', p_name_column, ') VALUES (?)');
...
BEGIN
    ...
    SET sql_stmt = CONCAT('UPDATE ', p_table_name, ' SET ');

    IF p_name_value IS NOT NULL THEN
        SET sql_stmt = CONCAT(sql_stmt, p_name_column, ' = ?, ');
    END IF;

    IF p_toggle_is_disabled IS NOT NULL THEN
        SET sql_stmt = CONCAT(sql_stmt, 'is_disabled = ?, ');
    END IF;

    SET sql_stmt = LEFT(sql_stmt, LENGTH(sql_stmt) - 2);
    SET sql_stmt = CONCAT(sql_stmt, ' WHERE ', p_id_column, ' = ?');
...
BEGIN
    ...
    SET sql_check = CONCAT(
        'SELECT COUNT(*) INTO @ref_count FROM products WHERE ', p_product_column, ' = ?'
    );
    ...
    IF ref_count > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot delete: entity is referenced by products';
    END IF;

    SET sql_delete = CONCAT(
        'DELETE FROM ', p_table_name, ' WHERE ', p_id_column, ' = ? AND is_disabled = TRUE'
    );
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM accounts WHERE account_id = p_account_id AND is_disabled = FALSE
    ) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Account not found or disabled';
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM accounts a WHERE a.account_id = p_account_id AND a.is_disabled = FALSE
    ) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Account not found or is disabled';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM user_profiles up WHERE up.account_id = p_account_id
    ) THEN
...
VALUES (
        'admin@shop.com',
        '$2a$12$SEctUhr.bUSCsDT/O0h7y.PxorwaTy7sgKQUCrSsXUtyb.b5MF2zG',
        TRUE
    ),
    (
        'user1@gmail.com',
